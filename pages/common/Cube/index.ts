...
class Cube {
  ...
  initialVertices(width: number, height: number, depth: number) {
    const w = width / 2;
    const h = height / 2;
    const d = depth / 2;
  ...
  initialCube(){
    ...
    const vertices = this.initialVertices(this.width, this.height, this.depth);
    const indices = this.initialIndices();
    const vertexColors = this.initialVertexColors();

    const buffer = gl.createBuffer();
    gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, buffer);
    gl.bufferData(gl.ELEMENT_ARRAY_BUFFER, indices, gl.STATIC_DRAW);

    createProgramAttribute(gl, program, 3, vertices, "aPosition", gl.FLOAT);
